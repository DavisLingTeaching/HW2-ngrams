...
def testGenerate():
    ...
    try: 
        ...
        sys.stderr.write('You need to implement the generate function\n')
        sys.exit(1)


    assert len(model.generate(5)) == 5, f"Not generating the correct number of"\
    " sentences"
    assert '<s> <s>' in model.generate(5)[0], f"Not generating from start "\
    "symbols"
    assert '</s>' in model.generate(5)[0], f"Not ending sentences"
